...
drogon::Task<drogon::HttpResponsePtr> CellUpdateController::updateCell(drogon::HttpRequestPtr req)
{
    ...
    try
    {
        ...
        try
        {
            CellUpdateService service;
            const WriteResult result = co_await service.update(parsed);
            const std::string dbName = parsed.payload.isMember("dbName") && parsed.payload["dbName"].isString()
                                           ? parsed.payload["dbName"].asString()
                                           : std::string();
            co_return makeSuccessResponse(result.rowId, dbName, result.extra);
        }
        catch (const CellUpdateError &e)
        {
    ...
    {
        LOG_ERROR(std::string("updateCell fatal error: ") + e.what());
        co_return makeErrorResponse("internal",
                                    "Internal error: " + std::string(e.what()),
                                    k500InternalServerError);
    }
}

CellUpdateController::ParsedRequest CellUpdateController::parseMultipartRequest(drogon::HttpRequestPtr req) const
{
...
drogon::HttpResponsePtr CellUpdateController::makeSuccessResponse(int64_t rowId,
                                                                  const std::string &dbName,
                                                                  const Json::Value &dataExtra)
{
    Json::Value root;
    root["ok"] = true;
    root["data"]["rowId"] = static_cast<Json::Int64>(rowId);
    if (!dbName.empty())
    {
        root["data"]["dbName"] = dbName;
    }
...
drogon::HttpResponsePtr CellUpdateController::makeErrorResponse(const std::string &code,
                                                                const std::string &message,
